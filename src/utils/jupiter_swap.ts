...
/**
 * Calculate required USDC amount to receive minimum 250 JUP
 * Includes 5% buffer to ensure at least 250 JUP is received
 * @returns Object containing calculated USDC amount and price info
 */
export async function calculateRequiredUsdcForMinimumJup(): Promise<{
  usdcAmount: number;
  jupPrice: number;
  targetJupAmount: number;
  minimumJupOutput: number;
  ...
  // Calculate: targetJupAmount JUP * jupPriceUSD = requiredUsdc
  // USDC is pegged to USD, so 1 USDC = 1 USD
  const requiredUsdc = targetJupAmount * jupPrice;
 ...
 *
 * NOTE: This function automatically calculates the required input amount
 * to receive at least 250 JUP (with 5% buffer) for both SOL and USDC.
 *
 * @param userPublicKey - User's public key
 * @param inputToken - Input token ticker ('SOL' or 'USDC')
 ...
 * @param slippageBps - Slippage in basis points (default 50 = 0.5%)
 * @returns Transaction, expected output amount, quote, and calculated input amount
 */
export async function buildJupiterSwapTransaction(
  ...
  _amount: number,
  slippageBps: number = 50
): Promise<{ transaction: VersionedTransaction; expectedOutput: number; quote: QuoteResponse; calculatedInputAmount: number }> {
  const inputMint = TOKEN_ADDRESS[inputToken];
  const outputMint = TOKEN_ADDRESS[outputToken];
  ...
  // Calculate required input amount based on token type for minimum 250 JUP (with 5% buffer)
  let calculatedInputAmount: number;

  if (inputToken === 'SOL') {
    const { solAmount } = await calculateRequiredSolForMinimumJup();
    calculatedInputAmount = solAmount;
    console.log('Calculated SOL amount for minimum 250 JUP:', calculatedInputAmount);
  } else if (inputToken === 'USDC') {
    const { usdcAmount } = await calculateRequiredUsdcForMinimumJup();
    calculatedInputAmount = usdcAmount;
    console.log('Calculated USDC amount for minimum 250 JUP:', calculatedInputAmount);
  } else {
    throw new Error(`Unsupported input token for auto-calculation: ${inputToken}`);
  }

  const jupiterApi = getJupiterClient();

  // 1. Get quote using calculated input amount
  const quote = await getJupiterQuote(inputToken, outputToken, calculatedInputAmount, slippageBps);
